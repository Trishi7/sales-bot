...
| It can read the sheet | ask `where are we with <a company in the tracker>?` | an answer citing the tab and the company |
| It can write the sandbox | ask `when should we follow up with <a company with no date>?` | a "No deadline was set — setting … shout to change" post, and the date appearing in the sandbox's `Next Deadline (bot)` column |
